...
const {canonicalLabelId, displayLabel, loadLabelRegistry, registryForClient} = require('./label-registry');
...
const labelsOutputFile = path.join(root, 'src', 'generated', 'labels.ts');
...
function normalizeReviewDimension(dimension, fallbackStatus, fallbackDate) {
  const status = dimension?.status || fallbackStatus;

  if (!dimension) {
    return {
      status,
      statusId: canonicalLabelId(labelRegistry, 'review_statuses', status),
      statusLabel: displayLabel(labelRegistry, 'review_statuses', status),
...
function toCamelCard(card) {
  ...
  return {
    ...
    categoryId: canonicalLabelId(labelRegistry, 'categories', card.category),
    categoryLabel: displayLabel(labelRegistry, 'categories', card.category),
    level: card.level,
    levelId: canonicalLabelId(labelRegistry, 'levels', card.level),
    levelLabel: displayLabel(labelRegistry, 'levels', card.level),
    pageLink,
    shortIntuition: card.short_intuition,
    maturity: card.maturity,
    maturityId: canonicalLabelId(labelRegistry, 'maturity', card.maturity),
    maturityLabel: displayLabel(labelRegistry, 'maturity', card.maturity),
    ...
    postQuantumPostureId: canonicalLabelId(labelRegistry, 'post_quantum_postures', card.post_quantum_posture),
    postQuantumPostureLabel: displayLabel(labelRegistry, 'post_quantum_postures', card.post_quantum_posture),
    postQuantumPostureLink: '/docs/appendices/post-quantum-posture',
    confidenceModelType: card.confidence_model.type,
    confidenceModelId: canonicalLabelId(labelRegistry, 'confidence_models', card.confidence_model.type),
    confidenceModelLabel: displayLabel(labelRegistry, 'confidence_models', card.confidence_model.type),
    confidenceModelLink: '/docs/appendices/confidence-models',
    implementationRisk: card.implementation_risk,
    implementationRiskId: canonicalLabelId(labelRegistry, 'implementation_risks', card.implementation_risk),
    implementationRiskLabel: displayLabel(labelRegistry, 'implementation_risks', card.implementation_risk),
    requiresTrustedSetup: card.requires_trusted_setup,
    requiresTrustedSetupId: canonicalLabelId(labelRegistry, 'trusted_setup', card.requires_trusted_setup),
    requiresTrustedSetupLabel: displayLabel(labelRegistry, 'trusted_setup', card.requires_trusted_setup),
    auditability: card.auditability,
    auditabilityId: canonicalLabelId(labelRegistry, 'auditability', card.auditability),
    auditabilityLabel: displayLabel(labelRegistry, 'auditability', card.auditability),
    parameterSensitivity: card.parameter_sensitivity,
    parameterSensitivityId: canonicalLabelId(labelRegistry, 'parameter_sensitivity', card.parameter_sensitivity),
    parameterSensitivityLabel: displayLabel(labelRegistry, 'parameter_sensitivity', card.parameter_sensitivity),
...
fs.writeFileSync(
  ...
  `// This file is generated by scripts/generate-concept-card-data.js.\n` +
    ...
    `const labelRegistry = ${JSON.stringify(registryForClient(labelRegistry), null, 2)} as const;\n\n` +
...
console.log(
  `Generated ${path.relative(root, outputFile)} and ${path.relative(root, labelsOutputFile)} from ${cards.length} concept cards.`,
);
